...
class RerankRetriever:
    ...
    def build_index(self, qa_pairs: List[Tuple[str, str]]):
        ...
        # 优化文本表示方式 - 避免使用[SEP]
        self.qa_texts = [{"question": q, "answer": a} for q, a in qa_pairs]

        logging.info("✅ 数据准备完成.....")

    def _get_text_representation(self, qa: dict) -> str:
        """自定义文本表示方式"""
        # 更简洁且无冲突风险的表示方式
        return f"Q: {qa['question']} | A: {qa['answer'][:200]}"
    ...
    def search(self, query: str, top_k: int = 5, batch_size: int = 32) -> List[Tuple[float, Tuple[str, str]]]:
        ...
        try:
            ...
            # 准备检索对 - 使用元组而非列表避免多进程问题
            pairs = [
                (query, self._get_text_representation(qa))
                for qa in self.qa_texts
            ]
            ...
            logging.error(f"Rerank计算失败: {str(e)}")
            # 尝试优雅降级处理
            scores = [0.0] * len(self.qa_texts)
        ...
        for i in indices:
            qa_dict = self.qa_texts[i]
            # 返回原始问答对，无需分割
            results.append((float(scores[i]), (qa_dict['question'], qa_dict['answer'])))
...
class HybridRetriever:
    ...
    def __init__(self,
                 vector_model: str = mode_path,
                 rerank_model: str = rerank_path,
                 candidate_size: int = 100):
        """
        初始化混合检索器
        :param vector_model: 向量模型名称
        :param rerank_model: Rerank模型名称
        :param candidate_size: 初筛候选数量
        """
        self.vector_retriever = VectorRetriever(vector_model)
        self.rerank_retriever = RerankRetriever(rerank_model)
        self.qa_pairs = []
    ...
    def build_index(self, qa_pairs: List[Tuple[str, str]]):
        ...
        print(f'混合模式下的qa_pairs内容....：{qa_pairs}')
        # 分别构建两种索引
        self.vector_retriever.build_index(qa_pairs)
        self.rerank_retriever.build_index(qa_pairs)

    def search(self, query: str, top_k: int = 5) -> List[Tuple[float, Tuple[str, str]]]:
        """
        执行混合检索：向量召回 + Rerank精排
        :param query: 用户查询
        :param top_k: 最终返回数量
        :return: 包含(分数, (问题, 答案))的列表
        """
        ...
        # 步骤1: 向量初筛
        print(f"⚡ 向量初筛召回 {self.candidate_size} 候选...")
        vector_results = self.vector_retriever.search(query, top_k=self.candidate_size)

        # 提取初筛的问答对
        candidate_qas = [ (q, a) for _, (q, a) in vector_results]

        # 为候选集构建临时索引
        self.rerank_retriever.build_index(candidate_qas)

        # 步骤2: Rerank精排
        return self.rerank_retriever.search(query, top_k=top_k)
...
class QASystem:
    ...
    def __init__(self, qa_pairs: List[Tuple[str, str]], mode: str = "hybrid"):
        ...
        elif mode == "rerank": # 精排模型
            self.retriever = RerankRetriever()
        elif mode == "hybrid":# 混合模型
...
if __name__ == "__main__":
    ...
    rerank_system = QASystem(qa_database, mode="rerank")
    hybrid_system = QASystem(qa_database, mode="hybrid")
    ...
    #
    #     print("\n>>> 纯向量检索结果 <<<")
    #     vector_results = vector_system.query(question, top_k=5)
    #
    #     print("\n>>> 纯Rerank检索结果 <<<")
    #     rerank_results = rerank_system.query(question, top_k=3)
    #     #
    #     # print("\n>>> 混合检索结果 <<<")
    #     # hybrid_results = hybrid_system.query(question, top_k=3)
    ...
    # 纯Rerank性能
    start = time.time()
    _rerank = rerank_system.query(test_question, top_k=3)
    rerank_time = time.time() - start

    # 混合性能
    start = time.time()
    _hybrid = hybrid_system.query(test_question, top_k=3)
    ...
    print(f"  纯Rerank检索: {rerank_time * 1000:.2f}ms")
    print(f"  混合检索: {hybrid_time * 1000:.2f}ms")
    print(f"  混合检索比纯Rerank快 {((rerank_time - hybrid_time) / rerank_time) * 100:.1f}%")